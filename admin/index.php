		...
		if($action=="")
		{
			...
			else
			{
				...
				foreach($allThemeSites as $siteInfo)
				{
					...
					$blogID= $siteInfo['blogID'];
					
					$tempThemeArray[$themeName][] = $blogID;
//					${$themeName.'Count'}++;
				}
				...
				foreach($myThemes as $thisTheme => $themeObject)
				{
					...
					$templateName = $themeObject['Template'];
					// Get the count of blogs using this theme
					
					if(array_key_exists($templateName,$tempThemeArray))
					{
						$thisCount = count($tempThemeArray[$templateName]);
					}
